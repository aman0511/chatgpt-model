...
def finetune_model(fileid, suffix, model='davinci'):
    ...
    resp = requests.request(method='POST', url='https://api.openai.com/v1/fine-tunes', json=payload, headers=header)
    pprint(resp.json())
...
if __name__ == "__main__":
    file_path = infile("Enter file path")
    if not  file_path:
        file_path = "depression_prepared.jsonl"
    resp = file_upload(file_path)
